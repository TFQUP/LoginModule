...
//  VSPageMenu
//
//  Created by 乐升平 on 17/10/26. https://github.com/SPStore/VSPageMenu
...
typedef NS_ENUM(NSInteger, VSPageMenuTrackerStyle) {
    VSPageMenuTrackerStyleLine = 0,                  // 下划线,默认与item等宽
    VSPageMenuTrackerStyleLineLongerThanItem,        // 下划线,比item要长(长度为item的宽+间距)
    VSPageMenuTrackerStyleLineAttachment,            // 下划线“依恋”样式，此样式下默认宽度为字体的pointSize，你可以通过trackerWidth自定义宽度
    VSPageMenuTrackerStyleRoundedRect,               // 圆角矩形
    VSPageMenuTrackerStyleRect,                      // 矩形
    VSPageMenuTrackerStyleTextZoom NS_ENUM_DEPRECATED_IOS(6_0, 6_0, "该枚举值已经被废弃，请用“selectedItemZoomScale”属性代替"), // 缩放(该枚举已经被废弃,用属性代替的目的是让其余样式可与缩放样式配套使用。如果你同时设置了该枚举和selectedItemZoomScale属性，selectedItemZoomScale优先级高于VSPageMenuTrackerStyleTextZoom
    VSPageMenuTrackerStyleNothing                    // 什么样式都没有
};

typedef NS_ENUM(NSInteger, VSPageMenuPermutationWay) {
    VSPageMenuPermutationWayScrollAdaptContent = 0,  // 自适应内容,可以左右滑动
    VSPageMenuPermutationWayNotScrollEqualWidths,    // 等宽排列,不可以滑动,整个内容被控制在pageMenu的范围之内,等宽是根据pageMenu的总宽度对每个按钮均分
    VSPageMenuPermutationWayNotScrollAdaptContent    // 自适应内容,不可以滑动,整个内容被控制在pageMenu的范围之内,这种排列方式下,自动计算item之间的间距,itemPadding属性无效
};

typedef NS_ENUM(NSInteger, VSPageMenuTrackerFollowingMode) {
    VSPageMenuTrackerFollowingModeAlways = 0,   // 外界scrollView拖动时，跟踪器时刻跟随外界scrollView移动
    VSPageMenuTrackerFollowingModeEnd,     // 外界scrollVie拖动结束后，跟踪器才开始移动
    VSPageMenuTrackerFollowingModeHalf     // 外界scrollView拖动到一半时，跟踪器开始移动
};
...
@class VSPageMenu,VSPageMenuButtonItem;

@protocol VSPageMenuDelegate <NSObject>

@optional
// 若以下2个代理方法同时实现了，只会走第2个代理方法（第2个代理方法包含了第1个代理方法的功能）
- (void)pageMenu:(VSPageMenu *)pageMenu itemSelectedAtIndex:(NSInteger)index;
- (void)pageMenu:(VSPageMenu *)pageMenu itemSelectedFromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex;

// 右侧的功能按钮被点击的代理方法
- (void)pageMenu:(VSPageMenu *)pageMenu functionButtonClicked:(UIButton *)functionButton;
@end

@interface VSPageMenu : UIView
 ...
 *  传递数据
 *
 *  @param items    数组 (数组元素可以是NSString、UIImage类型、VSPageMenuButtonItem类型，其中VSPageMenuButtonItem相当于一个模型，可以同时设置图片和文字)
...
#else
@property (nonatomic, readonly) VSPageMenuTrackerStyle trackerStyle;
#endif

@property (nonatomic, assign) VSPageMenuPermutationWay permutationWay; // 排列方式
...
@property (nonatomic, assign) VSPageMenuTrackerFollowingMode trackerFollowingMode; // 跟踪器的跟踪模式
...
// 选中的item缩放系数，默认为1，为1代表不缩放，[0,1)之间缩小，(1,+∞)之间放大，(-1,0)之间"倒立"缩小，(-∞,-1)之间"倒立"放大，为-1"倒立不缩放",如果依然使用了废弃的VSPageMenuTrackerStyleTextZoom样式，则缩放系数默认为1.3
...
@property (nonatomic, weak) id<VSPageMenuDelegate> delegate;
...
- (void)insertItem:(nonnull VSPageMenuButtonItem *)item atIndex:(NSUInteger)itemIndex animated:(BOOL)animated;
...
- (void)setItem:(VSPageMenuButtonItem *)item forItemAtIndex:(NSUInteger)itemIndex; // 同时为指定item设置标题和图片
- (nullable VSPageMenuButtonItem *)itemAtIndex:(NSUInteger)itemIndex; // 获取指定item

- (void)setContent:(id)content forItemAtIndex:(NSUInteger)itemIndex; // 设置指定item的内容，content可以是NSString、UIImage或VSPageMenuButtonItem类型
- (id)contentForItemAtIndex:(NSUInteger)itemIndex; // 获取指定item的内容，该方法返回值可能是NSString、UIImage或VSPageMenuButtonItem类型
...
// 设置功能按钮的内容，content可以是NSString、UIImage或VSPageMenuButtonItem类型
...
/* 1.让跟踪器时刻跟随外界scrollView滑动,实现了让跟踪器的宽度逐渐适应item宽度的功能;
   2.这个方法用于外界的scrollViewDidScroll代理方法中，如
    ...
    3.如果外界设置了VSPageMenu的属性"bridgeScrollView"，那么外界就可以不用在scrollViewDidScroll方法中调用这个方法来实现跟踪器时刻跟随外界scrollView的效果,内部会自动处理; 外界对VSPageMenu的属性"bridgeScrollView"赋值是实现此效果的最简便的操作
 */
...
// 默认NO;关闭跟踪器的跟随效果,在外界传了scrollView进来或者调用了moveTrackerFollowScrollView的情况下,如果为YES，则当外界滑动scrollView时，跟踪器不会时刻跟随,只有滑动结束才会跟随;  3.4.0版本开始被废弃，但是依然能使用,使用后相当于设置了VSPageMenuTrackerFollowingModeEnd枚举值
...
- (void)setFunctionButtonWithItem:(VSPageMenuButtonItem *)item forState:(UIControlState)state NS_DEPRECATED_IOS(6_0, 6_0, "Use -setFunctionButtonContent:forState:");
- (void)setItem:(VSPageMenuButtonItem *)item forItemIndex:(NSUInteger)itemIndex NS_DEPRECATED_IOS(6_0, 6_0, "Use -setItem:forItemAtIndex:");
...
@interface VSPageMenuButtonItem : NSObject

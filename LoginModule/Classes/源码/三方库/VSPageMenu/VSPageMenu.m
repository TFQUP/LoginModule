...
@interface VSPageMenuScrollView : UIScrollView
@end

@implementation VSPageMenuScrollView
...
@interface VSPageMenuLine : UIImageView
@property (nonatomic, copy) void(^hideBlock)(void);

@end

@implementation VSPageMenuLine
...
@interface VSPageMenuButton : UIButton
...
@implementation VSPageMenuButton
...
- (CGRect)titleRectImageAtLeftForContentRect:(CGRect)contentRect titleRect:(CGRect)titleRect imageRect:(CGRect)imageRect {
    ...
    titleOrigin.x = titleOrigin.x + _imageTitleSpace * 0.6;
...
@interface VSPageMenu()
@property (nonatomic, assign) VSPageMenuTrackerStyle trackerStyle;
...
@property (nonatomic, weak) VSPageMenuScrollView *itemScrollView;
@property (nonatomic, weak) VSPageMenuButton *functionButton;
@property (nonatomic, strong) NSMutableArray *buttons;
@property (nonatomic, strong) VSPageMenuButton *selectedButton;
...
@implementation VSPageMenu


#pragma mark - public

+ (instancetype)pageMenuWithFrame:(CGRect)frame trackerStyle:(VSPageMenuTrackerStyle)trackerStyle {
    VSPageMenu *pageMenu = [[VSPageMenu alloc] initWithFrame:frame trackerStyle:trackerStyle];
    return pageMenu;
}

- (instancetype)initWithFrame:(CGRect)frame trackerStyle:(VSPageMenuTrackerStyle)trackerStyle {
    if (self = [super init]) {
        self.frame = frame;
...
- (void)setItems:(NSArray *)items selectedItemIndex:(NSInteger)selectedItemIndex {
    ...
    NSAssert(selectedItemIndex <= items.count-1, @"selectedItemIndex 大于了 %lu",items.count-1);
    _items = items.copy;
    ...
    if (self.buttons.count) {
        for (VSPageMenuButton *button in self.buttons) {
    ...
    for (int i = 0; i < items.count; i++) {
        id object = items[i];
        NSAssert([object isKindOfClass:[NSString class]] || [object isKindOfClass:[UIImage class]] || [object isKindOfClass:[VSPageMenuButtonItem class]], @"items中的元素类型只能是NSString、UIImage或VSPageMenuButtonItem");
    ...
    if (self.buttons.count) {
        ...
        VSPageMenuButton *selectedButton = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:selectedButton];

        // VSPageMenuTrackerStyleTextZoom和VSPageMenuTrackerStyleNothing样式跟tracker没有关联
...
- (void)insertItemWithTitle:(NSString *)title atIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    self.insert = YES;
    NSAssert(itemIndex <= self.items.count, @"itemIndex超过了items的总个数“%lu”",(unsigned long)self.items.count);
...
- (void)insertItem:(VSPageMenuButtonItem *)item atIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    self.insert = YES;
    NSAssert(itemIndex <= self.items.count, @"itemIndex超过了items的总个数“%lu”",(unsigned long)self.items.count);
    NSMutableArray *objects = self.items.mutableCopy;
...
- (void)removeItemAtIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    ...
    if (itemIndex < self.buttons.count) {
        VSPageMenuButton *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)removeAllItems {
    ...
    for (int i = 0; i < self.buttons.count; i++) {
        VSPageMenuButton *button = self.buttons[i];
        [button removeFromSuperview];
    }
...
- (void)setItem:(VSPageMenuButtonItem *)item forItemIndex:(NSUInteger)itemIndex {
    if (item == nil) return;
    if (itemIndex < self.buttons.count) {
        VSPageMenuButton *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)setItem:(VSPageMenuButtonItem *)item forItemAtIndex:(NSUInteger)itemIndex {
    [self setItem:item forItemIndex:itemIndex];
}

- (VSPageMenuButtonItem *)itemAtIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.items.count) {
        id object = [self.items objectAtIndex:itemIndex];
        NSAssert([object isKindOfClass:[VSPageMenuButtonItem class]],@"itemIndex对应的item不是VSPageMenuButtonItem类型，请仔细核对");
        return object;
    }
    return nil;
}
...
- (void)setContent:(id)content forItemIndex:(NSUInteger)itemIndex {
    ...
    if (itemIndex < self.buttons.count) {
        ...
        } else if ([content isKindOfClass:[VSPageMenuButtonItem class]]) {
            VSPageMenuButtonItem *item = (VSPageMenuButtonItem *)content;
            [button setTitle:item.title forState:UIControlStateNormal];
            [button setImage:item.image forState:UIControlStateNormal];
            button.imagePosition = item.imagePosition;
            button.imageTitleSpace = item.imageTitleSpace;
...
- (void)setFunctionButtonWithItem:(VSPageMenuButtonItem *)item forState:(UIControlState)state {
...
- (void)addButton:(NSInteger)index object:(id)object animated:(BOOL)animated {
    ...
    VSPageMenuButton *button = [VSPageMenuButton buttonWithType:UIButtonTypeCustom];
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        VSPageMenuButtonItem *item = (VSPageMenuButtonItem *)object;
        [button setTitle:item.title forState:UIControlStateNormal];
        [button setImage:item.image forState:UIControlStateNormal];
        button.imagePosition = item.imagePosition;
        button.imageTitleSpace = item.imageTitleSpace;
...
- (BOOL)haveOrNeedsTracker {
    if (self.trackerStyle != VSPageMenuTrackerStyleTextZoom && self.trackerStyle != VSPageMenuTrackerStyleNothing) {
        return YES;
    }
    return NO;
}
...
- (void)initialize {
    ...
    _selectedItemTitleFont = [UIFont systemFontOfSize:14];
    _unSelectedItemTitleFont = [UIFont systemFontOfSize:14];
...
- (void)setupSubViews {
    // 必须先添加分割线
    VSPageMenuLine *dividingLine = [[VSPageMenuLine alloc] init];
    ...
    VSPageMenuScrollView *itemScrollView = [[VSPageMenuScrollView alloc] init];
    ...
    VSPageMenuButton *functionButton = [VSPageMenuButton buttonWithType:UIButtonTypeCustom];
...
// 按钮点击方法
- (void)buttonInPageMenuClicked:(VSPageMenuButton *)sender {
    ...
    if (self.selectedButton != sender) {
        ...
        if (self.trackerStyle == VSPageMenuTrackerStyleTextZoom || _selectedItemZoomScale != 1) {
            if (labs(toIndex-fromIndex) >= 2) { // 该条件意思是当外界滑动scrollView连续的滑动了超过2页
                for (VSPageMenuButton *button in self.buttons) { // 必须遍历将非选中按钮还原缩放，而不是仅仅只让上一个选中的按钮还原缩放。因为当用户快速滑动外界scrollView时，会频繁的调用-zoomForTitleWithProgress:fromButton:toButton:方法，有可能经过的某一个button还没彻底还原缩放就直接过去了，从而可能会导致该按钮文字会显示不全，所以在这里，将所有非选中的按钮还原缩放
...
- (void)moveItemScrollViewWithSelectedButton:(VSPageMenuButton *)selectedButton {
...
// 移动跟踪器
- (void)moveTrackerWithSelectedButton:(VSPageMenuButton *)selectedButton {
...
// 功能按钮的点击方法
- (void)functionButtonClicked:(VSPageMenuButton *)sender {
...
- (void)prepareMoveTrackerFollowScrollView:(UIScrollView *)scrollView {
    ...
    switch (self.trackerFollowingMode) {
        case VSPageMenuTrackerFollowingModeAlways:
            // 这个方法才开始移动跟踪器
            ...
            break;
        case VSPageMenuTrackerFollowingModeHalf:{
            VSPageMenuButton *fromButton;
            VSPageMenuButton *toButton;
            if (progress > 0.5) {
...
- (void)moveTrackerWithProgress:(CGFloat)progress fromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex currentOffsetX:(CGFloat)currentOffsetX beginOffsetX:(CGFloat)beginOffsetX {
    ...
    if (self.trackerStyle == VSPageMenuTrackerStyleLine) {
    ...
    } else if (self.trackerStyle == VSPageMenuTrackerStyleLineAttachment) {
    ...
    } else if (self.trackerStyle == VSPageMenuTrackerStyleTextZoom || self.trackerStyle == VSPageMenuTrackerStyleNothing) {
        // 缩放文字
        if (_selectedItemZoomScale != 1) {
            [self zoomForTitleWithProgress:progress fromButton:fromButton toButton:toButton];
        }
    } else if (self.trackerStyle == VSPageMenuTrackerStyleRoundedRect) {
        newCenter.x = fromButton.center.x + xDistance * progress;
...
- (void)setTrackerStyle:(VSPageMenuTrackerStyle)trackerStyle {
    _trackerStyle = trackerStyle;
    switch (trackerStyle) {
        case VSPageMenuTrackerStyleLine:
        case VSPageMenuTrackerStyleLineLongerThanItem:
        case VSPageMenuTrackerStyleLineAttachment:
            self.tracker.backgroundColor = _selectedItemTitleColor;
            break;
        case VSPageMenuTrackerStyleRoundedRect:
        case VSPageMenuTrackerStyleRect:
            ...
            _trackerHeight = 0;
            break;
        case VSPageMenuTrackerStyleTextZoom:
            // 此样式下默认1.3
...
- (void)setSelectedItemIndex:(NSInteger)selectedItemIndex {
    _selectedItemIndex = selectedItemIndex;
    if (self.buttons.count) {
        VSPageMenuButton *button = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:button];
    }
}

- (void)setDelegate:(id<VSPageMenuDelegate>)delegate {
    if (delegate == _delegate) {return;}
    _delegate = delegate;
    if (self.buttons.count) {
        VSPageMenuButton *button = [self.buttons objectAtIndex:_selectedItemIndex];
...
- (void)setPermutationWay:(VSPageMenuPermutationWay)permutationWay {
    _permutationWay = permutationWay;
    if (!_forceUseSettingSpacing) {
        if (_permutationWay == VSPageMenuPermutationWayNotScrollEqualWidths) {
...
- (void)setCloseTrackerFollowingMode:(BOOL)closeTrackerFollowingMode {
    ...
    if (closeTrackerFollowingMode) {
        self.trackerFollowingMode = VSPageMenuTrackerFollowingModeEnd;
    } else {
        self.trackerFollowingMode = VSPageMenuTrackerFollowingModeAlways;
    }
}
#pragma mark - getter
...
- (void)layoutSubviews {
    ...
    if (self.permutationWay == VSPageMenuPermutationWayNotScrollAdaptContent && diff < 0) {
    ...
    [self.buttons enumerateObjectsUsingBlock:^(VSPageMenuButton *button, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.permutationWay == VSPageMenuPermutationWayScrollAdaptContent) {
        ...
        } else if (self.permutationWay == VSPageMenuPermutationWayNotScrollEqualWidths) {
            // 求出外界设置的按钮宽度之和
...
- (void)resetupTrackerFrameWithSelectedButton:(VSPageMenuButton *)selectedButton {
    CGFloat trackerX;
    CGFloat trackerY;
    CGFloat trackerW;
    CGFloat trackerH;
...
@implementation VSPageMenuButtonItem

+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image {
    VSPageMenuButtonItem *item = [[VSPageMenuButtonItem alloc] initWithTitle:title image:image imagePosition:SPItemImagePositionDefault];
    return item;
}

+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image imagePosition:(SPItemImagePosition)imagePosition {
    VSPageMenuButtonItem *item = [[VSPageMenuButtonItem alloc] initWithTitle:title image:image imagePosition:imagePosition];
    return item;
}

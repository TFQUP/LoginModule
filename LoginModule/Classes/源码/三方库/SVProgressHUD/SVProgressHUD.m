...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        NSURL *url = [bundle URLForResource:@"VSDKResources" withExtension:@"bundle"];
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    self.frame= [[[UIApplication sharedApplication] windows] objectAtIndex:0].bounds;
#else
    if (self.viewForExtension) {
...
+ (NSTimeInterval)displayDurationForString:(NSString*)string {
    CGFloat minimum = MAX((CGFloat)string.length * 0.06 + 0.4, [self sharedView].minimumDismissTimeInterval);
...
#pragma mark - 解决在iOS8 多次显示的时候 文字颜色越来越深
